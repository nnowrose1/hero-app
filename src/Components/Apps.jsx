...
const Apps = () => {
  ...
  <div className='text-center pb-10'>
    ...
    <div className='container mx-auto flex justify-between items-center mt-11 mb-6'>
  <p className='font-semibold text-2xl text-black'>({search === '' ?(appData.length) : matchedApp.length > 0 ? (matchedApp.length) : (0) }) Apps Found</p>
    <form onSubmit={handleSubmit} className='relative flex gap-2.5 items-center'>
        <span className='absolute left-3 text-[#627382]'><Search></Search></span>
        <input className='text-[#627382] py-3 rounded-sm pr-65 pl-10 outline-1 outline-gray-400' type="text" value={search} onChange={handleSearch} placeholder='search Apps' />
    ...
    <div  className='container mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-20'>
        {
        search === '' ? appData.map(app => <EachApp key={app.id} app={app}></EachApp>) :
